...
- **`GET /node-status` can no longer 500 — the node name stays on screen permanently.** Reported from production: the endpoint intermittently crashed (browser console full of 500s), so the SPA had no node data and the hero chip / info-card node row never rendered — the node name only ever appeared inside the failure banner. Every internal failure path is now contained: a crash in the health layer degrades to `unknown` (no player banner, technical detail kept admin-only, warning logged), `ResolveServerNodeAction` wraps its DB writes and falls back to the locally-known node when Pelican is down (a failed node mirror no longer erases a known placement), and the daemon-credentials refresh survives schema/DB errors (e.g. a prod install missing the token columns migration) instead of bubbling a QueryException.
