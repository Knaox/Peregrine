 ...
 *                  no variable to redirect). Delegated to AdjacentPortResolver,
 *                  which assigns the node's free allocation at that exact port
 *                  (add) or relocates the game onto a free consecutive pair
 *                  (change), then restarts.
 *
...
class QueryAccessResolver
{
    ...
    public function __construct(
        ...
        private AdjacentPortResolver $adjacent,
    ) {}
    ...
    public function resolve(Server $server): array
    {
        ...
        return match ($plan['kind']) {
            ...
            'adjacent' => $this->adjacent->resolve($server, (int) ($plan['offset'] ?? 1), $context),
