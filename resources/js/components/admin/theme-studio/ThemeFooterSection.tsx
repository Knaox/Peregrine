...
export function ThemeFooterSection({ draft, onField }: ThemeFooterSectionProps) {
    ...
    return (
        <div className="flex flex-col gap-4">
            <ToggleField
                label={t('theme-studio:fields.theme_footer_enabled', 'Show footer')}
            ...
            {draft.theme_footer_enabled && (
                <>
                    <TextareaField
                        label={t('theme-studio:fields.theme_footer_text', 'Footer text')}
                        value={draft.theme_footer_text}
                        rows={2}
                        placeholder={t('theme-studio:fields.theme_footer_text_placeholder',
                    ...
                    <div className="flex flex-col gap-2">
                        <span className="text-[11px] font-medium tracking-wide text-[var(--color-text-secondary)]">
                            {t('theme-studio:fields.theme_footer_links', 'Footer links')}
                        </span>
                        {draft.theme_footer_links.length === 0 && (
                            <p className="text-[11px] text-[var(--color-text-muted)]">
                                {t('theme-studio:fields.theme_footer_links_empty', 'No links yet.')}
                            </p>
                        )}
                        <div className="flex flex-col gap-2">
                            {draft.theme_footer_links.map((link, i) => (
                                <div key={i} className="flex items-center gap-2">
                                    <input
                                        type="text"
                                        value={link.label}
                                        placeholder={t('theme-studio:fields.theme_footer_link_label', 'Label')}
                        ...
                        >
                            + {t('theme-studio:fields.theme_footer_link_add', 'Add link')}
                        </button>
                    </div>
                </>
            )}
