...
export function ThemePagesSection({ draft, onField }: ThemePagesSectionProps) {
    ...
    return (
        <div className="flex flex-col gap-4">
            <ToggleField
                label={t('theme-studio:fields.theme_page_console_fullwidth', 'Console — full-width')}
                value={draft.theme_page_console_fullwidth}
                description={t('theme-studio:fields.theme_page_console_fullwidth_help',
                    'Removes side padding around the terminal so it stretches edge-to-edge.',
                )}
                onChange={(v) => onField('theme_page_console_fullwidth', v)}
            />
            <ToggleField
                label={t('theme-studio:fields.theme_page_files_fullwidth', 'Files — full-width')}
                value={draft.theme_page_files_fullwidth}
                description={t('theme-studio:fields.theme_page_files_fullwidth_help',
                    'Removes side padding around the file browser for denser columns.',
                )}
                onChange={(v) => onField('theme_page_files_fullwidth', v)}
            />
            <ToggleField
                label={t('theme-studio:fields.theme_page_dashboard_expanded', 'Dashboard — 4 columns on ultra-wide')}
                value={draft.theme_page_dashboard_expanded}
                description={t('theme-studio:fields.theme_page_dashboard_expanded_help',
